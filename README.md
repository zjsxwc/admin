...
- 使用了easyui这个商业版权的前端库，不能用于收费用途,我这里尝试使用js自己实现下面这些接口代替easyui
...
```
- rbac里每个操作都没有使用事务，于是操作失败也不能rollback恢复，所以存在丢失数据情况,我这里尝试对批量操作加事务

- 这个版本beego不支持数据库外键关联操作，虽然大部分人也用不上，但不能没有啊
